...
一次补丁发布：一项计划器默认值切换，以及恢复一项被依赖升级悄然关闭的 0.13.0 功能。

**因子化 `count()` 重写现已默认开启。** 符合条件的单子句 `count()`-over-join
（包括 0.13.0 恢复的 `!=` 容斥形式）不再物化连接即可完成计数。LSQB sf0.1（SQLite，
release）实测，两条路径均返回 LDBC 官方计数：**q1 72.4 s → 1.05 s（约 69×）**，
**q6 42.1 s → 0.31 s（约 134×）**。**答案不会改变** —— 该重写仅在可证明精确的
分解上触发，对任何无法验证的形状一律放弃，被放弃的查询与此前逐字节一致 —— 改变的是执行
计划。如需恢复此前行为，调用 `db.set_query_factorization(false)`。

**0.13.0 的「预期记号」解析提示在当前依赖下重新生效。** pest 2.8.0 将解析尝试跟踪改为
可选且默认关闭；由于我们的依赖声明是脱字号形式的 `2.7.9`，任何全新解析依赖的使用者都会
在 2.8.x 上构建 0.13.0 —— 此时 `help:` 记号列表与经过修正的光标位置会静默退回 0.13.0
之前的行为。我们自己的 CI 从未发现这一点 —— 提交的 lockfile 锁定在 2.7.x —— 因此现在新增
了一条定期的「全新解析」CI 通道。依赖下限提升至 `pest = "2.8"`，并且 mnestic 仅在**对已经
失败的脚本进行重新解析时**才启用 pest 的详细跟踪，因此成功的解析不付出任何代价，该进程级
设置也不会被遗留在你的应用中。

此外：`op_rand_uuid_v1` 改用版本中立的 `uuid::v1::Context` 别名，使本 crate 可在其声明
范围内的任意 uuid 版本上编译。
