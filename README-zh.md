...
**一次正确性发布：修复六个继承自上游 Cozo 的缺陷 —— 没有一个是本 fork 引入的回归，
它们在 fork 之前就已潜伏。** 其中两个属于调用方从外部无法察觉的静默失败：

- **`MultiTransaction::commit()` 会把失败的提交报告为成功。** 它丢弃了事务线程回传的
  `Result`，即使出错也返回 `Ok(())` —— 而 `cozo-bin` 的 HTTP `/transact` 端点正建立
  在其之上，对根本没有提交成功的事务回答 `200 {"ok": true}`。变更回调同样会为这些
  失败的提交触发，因此任何依据变更流做同步的系统都可能与数据库悄然分叉。
- **就地更新行时，全文索引的 posting 会泄漏。** 旧 posting 的删除被错误地以“该关系
  *同时*还带有普通 B-tree 索引”为前提，因此**只**带 FTS 索引的关系在对既有键做
  `:put` 时从不删除旧 posting：文档中已不再包含的词项仍会命中它，索引无界增长，
  BM25 统计量随之漂移 —— 在双文档语料上实测**分数误差达 55%**。
  **0.12.0 及此前的每个版本都受影响。**

**若你使用全文检索，请执行以下升级操作。** 写路径的修复只能阻止新的泄漏，
**无法清除已经写入的 posting**。只带 FTS 索引、且曾被就地更新过的关系，其索引
*当下即已受影响*，仅仅升级并不能修复它。请用新增的 `::reindex` 重建一次：

```
::reindex my_relation
```

`::reindex` 在单个写事务中就地重建关系的 HNSW / FTS / LSH 索引，依据的是每个索引
**自身存储的 manifest**，而非重新构造的配置 —— 这一点对 LSH 尤为关键：其 manifest
只保留推导出的分带几何结构，却不保留产生该结构的权重，因此“先删后建”会交还一个
召回率特征与你当初所要求的并不相同的索引。它同时也是批量导入路径的修复手段：这些
路径并不维护上述索引，`import_from_backup` 过去会静默地令其失效，现在则与
`import_relations` 一样发出告警，且两处告警都指向 `::reindex`，而不再要求你手工
复原原始的 `::hnsw`/`::fts` 创建脚本。

另外修复了两个非默认存储后端：`newrocksdb` 从未真正开启乐观冲突检测（两个事务可以
读取同一个键、都写入、且都提交成功 —— 一次已确认的写入被静默丢失）；`sled` 的
`del()` 从不真正删除（上游 **#306**）。两者现已在 CI 中运行事务契约测试套件。

完整内容（含 FTS 条目的结果变化说明）见
