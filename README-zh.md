...
- **存储 / 命名查询** —— `::query create/list/show/run/remove` 可持久化复用的只读规则，
...
本版本修正了字符串解析、嵌套 JSON 转换和单词 FTS
前缀查询。升级前请检查以下结果变化：

- **破坏性变更（结果）：** 双引号字符串 `"a\nb"` 现在包含换行符。需要保留反斜杠时，
  使用 `_"a\nb"_` 或绑定参数。字符串两端的空白与注释样式文本会被保留，需要去除时
  请显式处理。原始字符串的下划线围栏必须匹配并紧贴引号；存储查询的正文也需要检查。
- **破坏性变更（结果）：** 嵌套 UUID 改为字符串，字节改为 base64 字符串，标量无穷大
  改为 `"INFINITY"` / `"NEGATIVE_INFINITY"`。此规则也适用于新写入的 `Json` 列和
  `to_string`。已存储的 JSON 不会重写；请仅迁移类型已知的字段或调整使用方。
  旧无穷大转换成的 null 无法恢复。
- FTS 前缀应用索引配置的大小写转换和 ASCII 折叠过滤器：小写索引上的 `Di*` 可以匹配
  `Diwank`，无需重建索引。前缀匹配已索引的词（包括词干）；完整词仍使用完整分析流程。
  若需保留大小写或重音差异，请使用未配置相应过滤器的索引。
- `Di*^3` 等整数 FTS 权重不再触发 panic。

不需要迁移存储格式或发布新桥接版本。Python 原生类型转换保持不变；`\uXXXX` 转义
仍仅支持 BMP 字符，非 BMP 字符请直接使用原文或绑定参数。临时字符串迁移警告计划在
0.19.0 移除。

完整升级指导见 [`CHANGELOG-FORK.md`](CHANGELOG-FORK.md)。
