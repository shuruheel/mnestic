...
**这个缺陷是继承而来的，其历史与 Cozo 的时间穿梭一样久。** 四处之中有三处，在分叉点
（`481af05`，2024-12-04）就已是逐字相同的上游代码：`@` 选择器（`expr2vld_spec`）、
`validity(...)` 构造函数（`op_validity`），以及最糟糕的写路径本身 —— `ColType::Validity`
的 `DataValue::List` 分支与上游逐字节一致。三者共同经过的那个访问器同样来自上游：
`Num::get_int`，它会把任何整数值的浮点数强制转换为 `i64`。只有事务时间选择器是我们自己的，
而它并没有引入这一强制转换，而是继承了它 —— 0.10.0 只是把同一个访问器扩展到了一条新的时间轴上。
`Validity` 列与 `@` 时间穿梭，都是早于本分叉多年的 Cozo 既有特性；这并不是双时态特性
带来的问题。每一个带有 `Validity` 列的 CozoDB 数据库，都存在这个缺陷。
...
我们在任何地方都只找到了这一惯用写法的**唯一一个调用者**：**上游自己的 HNSW 测试**
（[`cozo-core/src/runtime/tests.rs`](cozo-core/src/runtime/tests.rs)）—— 我们原样继承了它，
上游至今仍在发布它。自这个测试存在以来，它一直在把 1970 年写进上游自己的有效时间轴。
它从未对该值做过断言，因此始终没有变红。既然它出现在我们所继承的测试套件里，
