...
/// layout (so the rewritten entry keeps the user's column order and arity) and
...
impl Analysis {
    ...
    fn extract(prog: &NormalFormProgram) -> Option<Analysis> {
        ...
        // Body: positive relation atoms, plus droppable pure-output unifications
        // (the `count(tup = [..])` shape). Anything else declines — including ANY
        // predicate (see the `!=` note below).
        ...
        for atom in &rule.body {
            match atom {
                ...
                // ANY predicate declines. The `!=` inclusion–exclusion extension
                // was removed for this release: it rested on
                // `count(x != y) = count(all) − count(x = y)`, i.e. that every
                // pair is either `x = y` or `x != y`. That is FALSE here for
                // numerically-equal Int/Float pairs — `op_neq` compares
                // numerically (`1 != 1.0` is false), but the `x = y` correction
                // term is built by JOINING the two vars, and `Int(1)`/`Float(1.0)`
                // never join under `Num::cmp`. Such a pair satisfies neither term,
                // so the rewrite over-counts. Declining keeps the naive count
                // exact; the extension can return with proper numeric-identity
                // reconciliation.
                NormalFormAtom::Predicate(_) => return None,
        ...
        //     atom, not a group key, not in another unification's binding or
        //     expression),
        //   * its expression is a non-erroring value packer (const / var /
        //     nested list of those) — never a computation that could fail,
        //   * every variable its expression references is bound by a relation
        //     atom (so dropping the unification cannot hide an unbound-variable
        //     error).
        ...
        for u in &unifs {
            ...
            // Every referenced variable must be bound by a relation atom —
            // otherwise dropping this unification could hide an unbound-variable
            // error. (This also subsumes the no-chains requirement below, since
            // another unification's binding is never a relation variable.)
            let refs = u.expr.bindings().ok()?;
            if !refs.is_subset(&rel_vars) {
                return None;
            }
    ...
    fn advisory(&self) -> Option<String> {
        ...
        Some(format!(
            "rule `?`: count over a materialized join; factorizes over separator \
             {{{sep_list}}} into {} components — see docs/specs/cardinality-algebra.md",
            comps.len()
        ))
    }
    ...
    fn synthesize(&self, disable_magic_rewrite: bool) -> Option<NormalFormProgram> {
        ...
        let entry_body = vec![NormalFormAtom::Rule(NormalFormRuleApplyAtom {
            name: top_name,
            args,
            span: self.span,
        })];
        let (entry_head, entry_aggr) = self.entry_head_layout(&r_var);
...
fn factorize_count(
    ...
    // (the join key resolves against a renamed column). It is rare and declines
    // safely.
