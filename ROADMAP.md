...
## What's already shipped (through 0.11.1)
...
- **Budgeted weighted traversal.** Cheapest-first expansion from a set of seeds, over non-negative edge weights, under a **global distinct-node budget** (and optionally a cost ceiling) and a depth bound, returning each reached node with its cost, parent and depth — parent pointers reconstruct any path without shipping every path. mnestic has the two halves and not their intersection: `BFS`/`DFS` take a global `limit` that early-terminates the search, but they are unweighted and hop-ordered; `ShortestPathDijkstra` is weighted but takes no budget and computes the full single-source shortest-path tree. `min_cost_k` does not close the gap either — it is a *per-group* bounded meet, so in one global group it caps cheapest **derivations**, never distinct **nodes**. Anything that walks a graph to fill a fixed context window needs exactly the missing primitive, and today has to reimplement level-by-level BFS in the host language, giving up the in-engine recursion that is the whole point. Deterministic by construction: admission ordered by a total order on cost, ties broken lexicographically — the same discipline that makes `min_cost_k` confluent.

  Two things make this more than one more fixed rule. It takes its edges either positionally or from a **cached graph projection** (`graph:`, 0.11.0), so repeated expansions skip the scan-and-rebuild cost the projection work just removed (positional input stays first-class — projections deliberately refuse transaction-time sources, so temporal relations feed the traversal directly). And the natural consumer is retrieval: `HybridSearch` grows an expansion mode that **seeds the traversal from the vector and keyword legs' own candidates** — fill the remaining budget with the cheapest graph context around what semantic and keyword recall found, with each admitted node's cost and parent reported in the per-leg detail. Today's `GraphLeg` requires every seed up front and ranks by unweighted hop distance; the expansion mode subsumes that, and it puts the primitive in reach of the Python / LangChain / LlamaIndex surfaces instead of leaving it a Datalog-only feature.
