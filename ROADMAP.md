...
## What's already shipped (through 0.11.0)
...
- **Antichain / skyline aggregate + interval primitives** (0.10.1): `register_bounded_meet_aggr` keeps, per group, the non-dominated (Pareto-frontier) set under a host-registered strict partial order (Rust-embedded-only v1), and `interval_overlaps` / `interval_coalesce` operate over half-open `[start, end)` list intervals. Specs: [`docs/specs/antichain-bounded-meet.md`](docs/specs/antichain-bounded-meet.md), [`docs/specs/cozoscript-extensions.md`](docs/specs/cozoscript-extensions.md).
