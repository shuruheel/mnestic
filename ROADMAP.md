...
## What's already shipped (through 0.10.7)
...
- **A cached graph projection.** A named, reusable adjacency structure over a stored relation — `::graph create G { edges: knows }` — that the graph FixedRules consume in place of a relation, invalidated when the underlying relation is written. Today every call to `ConnectedComponents`, `PageRank` or `ShortestPathDijkstra` re-scans the edge relation and rebuilds a CSR from scratch. On a 439k-edge LDBC SNB graph that is ~54 ms of scan plus ~50 ms of graph construction, against **6.4 ms** for twenty PageRank iterations: the algorithms are not the cost, the setup is, and it is paid again on every query. Two smaller correctness gaps live in the same corner and are each self-contained: `PageRank` accepts no node relation, so vertices with no edges are silently absent from its output (`ConnectedComponents` already takes one as an optional second input); and the underlying `page_rank` reports how many iterations it actually ran, which the FixedRule discards — so a run that hits the default `iterations: 10` cap without converging looks identical to one that converged.
