...
- **Deterministic greedy join reorder** (default **on**; opt out per-query with
  ...
  [`docs/specs/join-reorder.md`](docs/specs/join-reorder.md).
- **Automatic factorized `count()` rewrite** (opt-in, **default off**, behind
  ...
  `::explain`. Spec:
  [`docs/specs/cardinality-algebra.md`](docs/specs/cardinality-algebra.md).
...
- **Dominance bounded-meet — the antichain / skyline aggregate.**
  ...
  v1. Spec:
  [`docs/specs/antichain-bounded-meet.md`](docs/specs/antichain-bounded-meet.md).
- **Interval primitives** — `interval_overlaps(a, b)` (builtin) and
  ...
  [`docs/specs/cozoscript-extensions.md`](docs/specs/cozoscript-extensions.md) §3.4.
