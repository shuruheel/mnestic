...
_Created 2026-07-04. Status: **IMPLEMENTED 2026-07-04** (same-day build after sign-off; §6 matrix pinned in `cozo-core/tests/antichain_bounded_meet.rs`; build deviations recorded in the changelog — all narrowing or strengthening, none semantic). Previously: **SIGNED OFF 2026-07-04 — build authorized.** The owner delegated the technical review to research-backed diligence (not an area expert): decisions 1/3/5 were grounded in the skyline and order-theory literature at authoring, and a final prior-art check against **Soufflé's subsumption** — the closest existing engine implementation of dominance-based tuple deletion — confirmed both the design and its key dodge (§9): Soufflé's rule-level deletion must grapple with dominated tuples having already fired downstream inferences; this spec's aggregate-store framing confines dominance to the fold, riding the same approximate-in-recursion reading `min_cost_k` documents, and so never needs downstream retraction (the DRed-shaped problem this fork already declined as R3's incremental half)._

_Originally:_ **PROPOSED — awaiting owner sign-off; do not build before §8's decisions are signed.** This is the dedicated spec that `cozoscript-extensions.md` §3.1 requires before implementation: it resolves that section's four open questions (Q1 cap, Q2 law/probe, Q3 delivery surface, Q4 contracts) and replaces its honest-touch-list sketch with a buildable design. Grounded two ways: (a) against the shipped 0.10.0 `BoundedMeetStore` code (citations are `file:line` in `cozo-core/src/`), verified this session by a seven-dimension review panel plus an adversarial code review; (b) against the skyline literature for the three genuinely uncertain calls — the insert algorithm (BNL in-buffer maintenance is the canonical in-memory approach), the cap question (every principled bounded skyline in the literature is a *semantic* reduction — representative-k, k-dominant — never arrival-order truncation), and the tie-break composition rule (lexicographic composition is safe over strict **weak** orders; a genuinely partial clause is safe only in last position). Companion to [`provenance-semirings.md`](provenance-semirings.md) (R1 recorded exactly this as deferred: "Deferred: custom bounded-meet registration; configurable cap.") and [`cozoscript-extensions.md`](cozoscript-extensions.md) §3.1._
...
| Divergence cap: hard error after 4096 changed epochs (total count, hardened post-release from a resetting streak — equivalent today because the stratifier isolates bounded rules to their own in-stratum delta), cannot cap co-stratified recursion | `query/eval.rs:36`, `:141-167`, `:359-371` | verbatim |
...
- **Debug-build probes** (the R0b idempotence probe re-applies `update` and cannot check a comparator, so this category gets its own): on every insert, assert `!dominates(c, c)` (irreflexivity); on reject-direction comparisons that return true, assert the swapped call returns false (asymmetry) — coverage sufficient to keep a symmetric pair from ever corrupting the set, since a symmetric pair is caught in the reject loop before any eviction. Transitivity is not cheaply probeable and stays a documented obligation. A violation errors loudly (debug builds only), naming the aggregate — the R0b probe's `Result` style, not a panic.
...
**Decision:** `max_survivors` is a mandatory registration parameter; exceeding it mid-fold is a hard `bail!` naming the aggregate, the cap, and the group key — exactly the divergence-cap philosophy. No default (the registrant must think about it); no truncation mode. *Build-review precision note:* the bail fires on the **running** antichain, so whether a run bails can depend on candidate arrival order — the §3.3 confluence guarantee covers the output values of successful runs, not bail-vs-succeed (a deterministic mid-fold bail would require exactly the unbounded transient storage the guard exists to prevent).
...
## 8. Decisions (signed off 2026-07-04)
