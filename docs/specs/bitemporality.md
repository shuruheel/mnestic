...
| As-of read `@ <expr>` | grammar `cozoscript.pest:84` (`validity_clause = {"@" ~ expr}`); parse `parse/query.rs:1082-1098` (`expr2vld_spec`, const-evaluated: Num µs or Str only); carried as `valid_at: Option<ValidityTs>` on `FixedRuleArg::{Stored,NamedStored}` (`data/program.rs:372-390`) | `@ "NOW"` → current clock; `@ "END"` → **end of time** (`MAX_VALIDITY_TS = ValidityTs(Reverse(i64::MAX))`, `data/functions.rs` — the *final* state, including future-dated assertions; pinned by `data/tests/validity.rs:156-183`); numeric µs / ISO-8601 string otherwise. **A bare access with no `@` is a raw scan returning ALL versions.** |
| Skip-scan | dispatch `fixed_rule/mod.rs:94-101` → `runtime/relation.rs:372-385` (`skip_scan_all`) → backend `range_skip_scan_tuple` → `data/tuple.rs:60-84` (`check_key_for_validity`) | Per logical key: seek to first version with `ts ≤ valid_at`; if retraction → skip the whole key (seek to `TERMINAL_VALIDITY`, `data/functions.rs`); else emit and seek to next key. **At `@ "NOW"` this is one seek per key — the fast path we must preserve.** |
| Clock | `current_validity()` in `data/functions.rs` | Wall-clock microseconds, `Reverse`-wrapped. Captured **once per script** in `run_multi_transaction` (`runtime/db.rs`). **Per-script, not monotonic, not transaction-commit-scoped.** Two same-µs writes to one (key, vt) produce identical keys — the second silently replaces the first. |
...
**Grammar** — only the *interior* of `validity_clause` changes; the four attachment sites — the `fixed_relation_rel`, `fixed_named_relation_rel`, `relation_named_apply`, and `relation_apply` rules in `cozoscript.pest` — are untouched:
...
4. **Relationship to MindGraph's tombstones/supersession → live-graph adoption remains gated.** Updated 2026-09-07 by the MindGraph adoption feasibility ruling (`mindgraph/docs/plans/bitemporality-adoption.md`, D1/D6). Keep ordinary `node`/`edge` relations, application version ledgers, tombstones and `Supersedes`. The engine primitive is shipped; MindGraph's live graph is not thereby engine system-versioned. Application valid-time annotation has shipped separately (`mindgraph/docs/plans/bitemporal-valid-time.md`). Provenance-semirings R3 shipped as `:reconcile` without requiring unification.

   The current TxTime contract rejects the secondary/search indexes, triggers and graph-projection sources required by the live graph. MindGraph's relation-based JSON snapshot path cannot restore original transaction-time history, and a supported reversible in-place migration is absent. Each is an independent integration gate, not a universal restriction on temporal databases. Feeding current application version appenders into a shadow temporal table would retain their skipped-write gaps and would not establish exhaustive audit history.

   A future adoption proposal must settle indexed current reads, current-side trigger/projection behavior, history-preserving recovery and reversible enable/disable semantics. A pilot additionally needs a named system-time question and proven recovery. This records prerequisites; it neither authorizes a live-graph migration nor makes another ledger a build commitment.
