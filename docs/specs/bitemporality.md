...
| `:rm` | **Appends a retraction at commit-tt** on tt-only relations (flag byte; values snapshot the key's latest row at statement time). Never a physical delete (§4). *Step-3 deviation:* on **bitemporal** relations `:rm` is rejected in v1 — removal is a valid-time statement (`:put` with vt `"RETRACT"`); the `:rm {k, vt}` remap ships with step 4. Same-transaction put+rm of one key is rejected (tie). |
| `:update` / `:delete` / `:replace` | `:delete` = `:rm` with an existence assertion (fails on missing **or believed-deleted** keys). *Step-3 state:* `:update` rejected pending step-4 reads; `:replace` of an existing TxTime relation rejected outright (Cozo's `:replace` is destroy-and-recreate — it would drop history, which is not expressible on a tt axis). |
