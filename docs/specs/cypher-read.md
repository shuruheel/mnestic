...
_Status: **Implementation in progress — steps 2–4 landed (parser, translator, `run_cypher` entry); the surface is callable** (2026-06-27); steps 5–6 (TCK harness, Python binding + default-on) remain. Owner: TBD. Companion to the research briefing [`../research/cypher-read.md`](../research/cypher-read.md) (prior art, semantics, scoping), `../../ROADMAP.md` (Tier 1, sequenced **first**), and `../../DEVELOPMENT.md` §3.5. Grounded in the actual CozoScript target grammar (`cozo-core/src/cozoscript.pest`) and the `hybrid_search` translate-to-CozoScript precedent (`cozo-core/src/runtime/hybrid.rs`); citations are `file:line` in `cozo-core/src/`._
...
5. **TCK conformance harness** (vendored subset) + correctness-trap tests. *(next)*
